# Köögiviljade kasvatamise plaan

## Sisukord

**Peenrad ja kultuurid:**
- [Aia ülevaade](#aia-ülevaade) · [Peenarde jaotus 2026](#peenarde-jaotus-2026)
- [Peenar 1: Maasikad](#peenar-1-maasikad) · [Peenar 2: Maavitsalised](#peenar-2-maavitsalised-solanaceae)
- [Peenar 3: Herned + Oad + Juurviljad](#peenar-3-herned--oad--juurviljad) · [Peenar 4: Kurgid + Kõrvitsalised](#peenar-4-kurgid--kõrvitsalised)

**Eraldi kultuurid:**
- [Kartul](#kartul) · [Redis](#redis) · [Maapirn](#maapirn-jerusalem-artichoke) · [Porgand](#porgandi-kasvatamine)
- [Laugud (Allium)](#laugud-allium) · [Lehtviljad](#lehtviljad-lehtpeet-lehtkapsas-spinat)
- [Maitsetaimed](#maitsetaimed-ürdid) · [Kõrvitsalised](#kõrvitsalised-cucurbitaceae)

**Planeerimine ja meetodid:**
- [Ettekasvatamise kalender](#ettekasvatamise-kalender) · [Ettekasvatamise tingimused](#ettekasvatamise-tingimused)
- [Kaevamisvaba meetod](#kaevamisvaba-meetodi-põhimõtted) · [Külvikord](#külvikord-rotatsioon)
- [Kaaslaskultuurid](#kaaslaskultuuride-kokkuvõte) · [Haljasväetis](#haljasväetis-katvekultuurid)

**Praktilised juhendid:**
- [2026 kevadine tegevuskava](#2026-kevadine-tegevuskava) · [Seemnete hankimine](#seemnete-hankimise-nimekiri)
- [Kahjurid ja haigused](#kahjurid-ja-haigused) · [Märkmed](#märkmed)

---

## Aia ülevaade

**Aia tüüp:** Kaevamisvaba (no-dig)

| Parameeter | Väärtus |
|------------|---------|
| Peenarde arv | 4 tk |
| Peenra suurus | 1 × 9 meetrit |
| Peenra tüüp | Puust kast, täidetud mullaga |
| Peenarde vahe | 60 cm, kaetud oksalõikusmultsiga |

---

## Peenarde jaotus 2026

```
┌─────────────────────────────────────────────────────────────────┐
│                        PEENAR 1: MAASIKAD                       │
│  Maasikad (Honeoye, Sonsation, Asia, Faith) + küüslauk + kurgirohi │
└─────────────────────────────────────────────────────────────────┘
                              60 cm vahe
┌─────────────────────────────────────────────────────────────────┐
│                     PEENAR 2: MAAVITSALISED                     │
│      Tomatid + tšilli + füüsal + basiilik + peiulill      │
└─────────────────────────────────────────────────────────────────┘
                              60 cm vahe
┌─────────────────────────────────────────────────────────────────┐
│             PEENAR 3: HERNED + OAD + JUURVILJAD                 │
│  Herned + aedoad + porgand + sibul + porru + (salatid pärast)   │
└─────────────────────────────────────────────────────────────────┘
                              60 cm vahe
┌─────────────────────────────────────────────────────────────────┐
│               PEENAR 4: KURGID + KÕRVITSALISED                  │
│     Avamaakurk + suvikõrvits + patisson + till + päevalilled    │
└─────────────────────────────────────────────────────────────────┘
```

---

## Peenar 1: Maasikad

→ **Vaata eraldi faili:** [maasikad/maasikate-hooldus.md](../maasikad/maasikate-hooldus.md)

Sordid: Honeoye, Sonsation, Asia, Faith + kaaslaskultuurid (küüslauk, kurgirohi)

---

## Peenar 2: Maavitsalised (Solanaceae)

### Kultuurid

| Kultuur | Kogus | Vahe taimede vahel | Kõrgus | Tugi |
|---------|-------|-------------------|--------|------|
| Tomat | 5–6 taime | 50–60 cm | 1–2 m | Vaiad/puurid |
| Tšilli | 3–4 taime | 40–50 cm | 40–80 cm | Kerge tugi |
| Füüsal | 2–3 taime | 60–80 cm | 1–1,5 m | Vaiad |

### Tomatid

→ **Vaata detailset juhendit:** [tomatid/tomatite-hooldus.md](../tomatid/tomatite-hooldus.md)

**Testitud 2025:** Malle ⭐⭐⭐, Pille ⭐⭐⭐, Erk F1 ⭐⭐

**2026 sordid:**
- **Kasvuhoone:** Malle F1, Pille, Evelle, San Marzano
- **Avamaa:** Terma ⭐, Mato ⭐, Siive (madalakasvulised, varajased)

### Paigutus peenras (1 × 9 m)

```
┌─────────────────────────────────────────────────────────────────┐
│ Füüsal │    T    │    T    │    T    │    T    │    T    │ Tšilli │
│  (2 tk)  │         │         │         │         │         │ (3-4tk)│
│          │  Tomat  │  Tomat  │  Tomat  │  Tomat  │  Tomat  │        │
│  80 cm   │◄──60cm──►◄──60cm──►◄──60cm──►◄──60cm──►◄──60cm──►│  50 cm │
└─────────────────────────────────────────────────────────────────┘
         ↑                                                    ↑
    Basiilikus                                           Peiulill
    tomatite vahel                                       servades
```

### Kaaslaskultuurid

| Kaaslane | Kasu | Kuhu istutada |
|----------|------|---------------|
| **Basiilikus** | Peletab lehetäisid, parandab maitset | Tomatite vahele |
| **Peiulill (Tagetes)** | Peletab kahjureid, nematoodid | Peenra servad |
| **Porgand** | Kobestab mulda | Tomatite vahel (kui ruumi) |
| **Petersell** | Meelitab kasulikke putukaid | Servad |

### Mida MITTE istutada kõrvale

| Kultuur | Põhjus |
|---------|--------|
| Kapsas, lillkapsas | Konkurents toitainete pärast |
| Kartul | Jagavad haigusi (lehemädanik) |
| Kurk | Erinevad vajadused, haigused |
| Apteegitill (fennel) | Pärsib kasvu |

### Hoolduskalender

| Kuu | Tegevus |
|-----|---------|
| Veebruar lõpp | Tšilli ettekasvatuse algus |
| Märts keskpaik | Tomati ja füüsale ettekasvatuse algus |
| Aprill | Istikud suuremates pottides, karastamine |
| Mai lõpp | Karastamine õues (päeval) |
| Juuni algus | **Istutamine õue** (pärast öökülma ohtu) |
| Juuni | Tugede paigaldamine, esimene mulšimine |
| Juuni–august | Kõrvalvõsude eemaldamine (tomatil), kastmine, väetamine |
| August–september | Saagi korjamine |
| September lõpp | Tomatite ladvade näpistamine (marjad valmivad ära) |

---

## Peenar 3: Herned + Oad + Juurviljad

### Kultuurid

| Kultuur | Tüüp | Kogus/vahe | Märkused |
|---------|------|------------|----------|
| **Herned** | Põhi (kevad) | 2–3 m rida, 5–10 cm | Aprill külv, juulis valmis |
| **Aedoad (roniv)** | Põhi (suvi) | 2 m rida, 10–15 cm | Mai külv, juuli–sept saak |
| **Porgand** | Põhi (terve hooaeg) | 2–3 m, 4 rida | Aprill–okt, vt porgandi sektsioon |
| **Sibul** | Põhi | 1,5 m, 2 rida | Aprill–august, vt sibulaliste sektsioon |
| **Porru** | Põhi | 1 m rida | Mai–november, vt sibulaliste sektsioon |
| Salatid | Järelkülv | Pärast herneid | Juulist vabale alale |
| Murulauk | Serv | Peenra otsad | Püsik, peletab kahjureid |

### Oad

→ **Vaata detailset juhendit:** [oad/ubade-hooldus.md](../oad/ubade-hooldus.md)

**2026 sordid:**
- **Blauhilde** ⭐ – roniv, lilla/sinine, dekoratiivne
- **Neckargold** ⭐ – roniv, kollane, kiudevaba
- **Saxa** ⭐ – madal, varajane, kiudevaba
- **Jõgeva** ⭐ – põlduba, Eesti sort, külmakindel

### Paigutus peenras (1 × 9 m)

```
┌────┬────────────────┬────────────────┬─────────────────┬────┐
│Muru│ HERNED + OAD   │    PORGAND     │  SIBUL + PORRU  │Muru│
│lauk│ ~~~võrk~~~     │   ═══════      │   ○ ○ ○ ○ ○ ○   │lauk│
│    │ H H H O O O O  │   ═══════      │   (sibul)       │    │
│    │ (juulis valmis │   ═══════      │   ═══════════   │    │
│    │  → salatid)    │   ═══════      │   (porru)       │    │
│    │  ← 3 m →       │   ← 2,5 m →    │    ← 3 m →      │    │
└────┴────────────────┴────────────────┴─────────────────┴────┘
 30cm                                                     30cm

Pärast herneid (juuli): külva samale alale salatid või spinat!
```

### Miks see kombinatsioon töötab

| Seos | Selgitus |
|------|----------|
| Herned + Oad | Mõlemad liblikõielised – seovad lämmastikku, jagavad tuge |
| Porgand + Sibul | Klassikaline paar – sibul peletab porgandikärbest, porgand sibulakärbest |
| Herned → Salatid | Pärast herneid jääb lämmastikurikas muld → salatid kasvavad kiiresti |
| Murulauk servades | Püsik, peletab kahjureid, ei võta ruumi |

### Herned

→ **Vaata detailset juhendit:** [herned/herneste-hooldus.md](../herned/herneste-hooldus.md)

**Testitud 2025:** Looming ⭐⭐⭐, Aamisepp ⭐⭐

**2026 sordid:**
- **Looming** ⭐ – madal, palju kaunu, Eesti sort
- **Kelvedon Wonder** ⭐ – varajane, kompaktne
- **Ambassador** – magus, hea maitse

### Herneste ja ubade võrdlus

| Omadus | Herned | Ronivad oad | Madalad oad |
|--------|--------|-------------|-------------|
| Külviaeg | Aprill (külmakindel) | Mai keskpaik (ei talu külma) | Mai keskpaik |
| Saak algab | Juuni | Juuli | Juuli |
| Saagi kestus | 3–4 nädalat | 6–8 nädalat | 4–5 nädalat |
| Kõrgus | 0,5–2 m (sorditi) | 2–3 m | 40–50 cm |
| Korjamissagedus | Iga 2–3 päeva | Iga 2–3 päeva | Iga 3–4 päeva |

### Hoolduskalender

| Kuu | Kultuur | Tegevus |
|-----|---------|---------|
| Aprill | Herned | **Külv otse peenrasse** (külmakindel) |
| Aprill | Porgand | **Külv otse peenrasse** (muld min 7°C) |
| Aprill | Sibul | **Istikute istutamine** |
| Mai | Porru | **Istutamine** (istikud 15 cm sügavusse) |
| Mai keskpaik | **Aedoad** | **Külv otse peenrasse** (pärast öökülma ohtu) |
| Juuni | Aedoad | Toe paigaldamine ronivatele |
| Juuni | Herned | Saagi algus, regulaarne korjamine |
| Juuli | Herned | Saak lõpeb, **jäta juured mulda** → külva salatid! |
| Juuli | Aedoad | **Saagi algus** (ronivad ja madalad) |
| Juuli–sept | Aedoad | Korjamine iga 2–3 päeva |
| August | Sibul | **Koristamine**, kuivatamine |
| Sept–okt | Porgand | **Koristamine** |
| Sept–nov | Porru | **Koristamine** vastavalt vajadusele |

---

## Haljasväetis (katvekultuurid)

Pärast varajaste kultuuride (herned, kartul) koristamist on kasulik külvata haljasväetis, mis:
- Parandab mullastruktuuri
- Seob toitaineid (ei leostu välja)
- Pärsib umbrohtu
- Liblikõielised seovad lämmastikku

### 2025 testitud kombinatsioon ✓

Pärast herneid ja kartuleid (9. august) külvatud:

| Kultuur | Ladina nimi | Kasu |
|---------|-------------|------|
| **Inkarnaatristik** | Trifolium incarnatum | Seob lämmastikku, ilus punane õis |
| **Keerispea** | Phacelia tanacetifolia | Kobestab mulda, mesilaste lemmik |
| **Taikon** (taliredis) | Raphanus sativus | Sügavad juured, kobestab, "biodrenaaz" |

### Haljasväetise kasutamine

**Millal külvata:**
- Pärast varajaste kultuuride koristamist (juuli–august)
- Vähemalt 6–8 nädalat enne esimesi külmi

**Kuidas:**
1. Korista eelmine kultuur
2. Jäta juured mulda (liblikõieliste puhul)
3. Lisa õhuke kiht komposti (valikuline)
4. Külva haljasväetise seemned laiali
5. Riisuta kergelt mulda
6. Kasta sisse

**Talveks:**
- Lase külmuda (talvituvad osaliselt)
- Kevadel materjal kõduneb mulda
- Või niida enne talve ja jäta multšiks

### Haljasväetise seemnesegu

Lihtne segu ise kokku panna:
- 50% keerispea
- 30% inkarnaatristik (või valge ristik)
- 20% taikon

**Kogus:** ~15–20 g/m²

---

## Kartul

Kartul on klassikaline Eesti aiakultur, mis sobib hästi kaevamisvaba aia süsteemi. Pärast kartuleid on hea külvata haljasväetist.

### Testitud sordid 2025

| Sort | Tüüp | Küpsemine | Saak | Märkused |
|------|------|-----------|------|----------|
| **'Solist'** | Varajane | 67–82 päeva | Keskmine | Kollane liha, hea maitse |
| **'Annagrete'** | Varajane | 65–75 päeva | Keskmine | Kollane liha, sileda koorega |

---

### Varajane kartul jaanipäevaks ('Solist')

**Eesmärk:** Kartulid laual 24. juuniks!

**Ajakava:**

| Etapp | Aeg | Kestus |
|-------|-----|--------|
| Eelidandamine | Märtsi algus | 2–3 nädalat |
| Juurdumisaeg | Märtsi keskpaik | 3–4 nädalat |
| Mahapanekuks valmis | Aprilli lõpp – mai algus | – |
| Kasvuaeg mullas | 52–67 päeva | – |
| **Saak** | **~24. juuni** | – |

**Samm-sammult juhend:**

#### 1. Eelidandamine (märtsi algus)

```
┌─────────────────────────────────────────┐
│  ○ ○ ○ ○ ○ ○ ○ ○  ← mugulad ühekihiliselt│
│  ═══════════════  ← 4–5 cm turbamulda   │
│  ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓  ← kast               │
└─────────────────────────────────────────┘
```

- Aseta mugulad **4–5 cm turbamulla kihi peale**
- Ühekihiliselt, väikeste vahedega
- Koht: **valge, alla 20°C**

#### 2. Juurdumisaeg (märtsi keskpaik)

- Tõsta **jahedamasse, valgesse kohta**
- Lase mugulatel juurduda
- **NB:** Mugulaid tuleb kastist välja võtta **koos mullapalliga!**

#### 3. Maa ettevalmistamine (1–2 nädalat enne)

```
         katteloor
            ↓
    ░░░░░░░░░░░░░░░░░░
    ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓  ← must kile
    ════════════════════  ← muld
```

- Pane **must kile vaole peale** (soojendab maapinda!)
- Lisa **katteloor** kile peale

#### 4. Mahapaneku päev (apr lõpp – mai algus)

**Paar päeva enne:**
1. Kaeva vagu läbi
2. Tee vaod valmis
3. Pane must kile ja katteloor tagasi

**Mahapanekul:**
1. Lisa vagudesse **köögiviljaväetist**
2. Aseta mugulad vagudesse:
   - Sügavus: **10 cm**
   - Vahe: **30 cm**
3. Kata musta kile ja kattelooriga

#### 5. Hooldus

| Tegevus | Millal |
|---------|--------|
| Musta kile eemaldamine | Kui taimed hakkavad kile all kasvama |
| Katteloori eemaldamine | Kui taimed hakkavad katteloori tõstma |
| Muldamine/multšimine | Kui taimed 15–20 cm kõrged |

**⚠️ Tähtis:** Must kile tuleb **õigel ajal pealt ära** – muidu taimed põlevad!

#### 6. Saak (~24. juuni)

- Kasvuaeg mullas: 52–67 päeva
- Valmis, kui õied närbuvad / esimesed lehed kollaseks

---

### Kasvatamine kaevamisvabal meetodil

**Ettevalmistus:**
- Varakevadel (märts) pane mugulad idanema: jahe, valge koht
- Head idandid on 1–2 cm pikad, tugevad

**Istutamine:**

| Tegevus | Juhis |
|---------|-------|
| Aeg | Mai keskpaik (muld min 8°C, öökülmad möödas) |
| Sügavus | 10–15 cm (kaevamisvaba: multši alla) |
| Vahe reas | 30–40 cm |
| Ridade vahe | 60–70 cm |
| Varajased | Mai algus (kate vajalik öökülmade korral) |

**Kaevamisvaba meetod:**
1. Aseta idandatud mugulad komposti peale
2. Kata 20–30 cm paksuse heinamultšiga
3. Lisa multši juurde, kui taimed kasvavad
4. Korjamisel lükka multš kõrvale – mugulad on peal!

### Hooldus

| Kuu | Tegevus |
|-----|---------|
| Mai | Istutamine, esimene multšimine |
| Juuni | Multši lisamine (muldamise asemel) |
| Juuli | Varajaste koristamine (kui õied närbuvad) |
| August | Hilisemate koristamine |

**Muldamine vs multšimine:**
- Traditsiooniline: muld varrele
- Kaevamisvaba: hein/põhk varrele (sama efekt, lihtsam!)

### Koristamine

**Õige aeg:**
- **Varajased:** Kui taim õitseb ja lehed hakkavad kollaseks minema (~70 päeva)
- **Hilised:** Kui lehed on täiesti kolletunud/surnud

**Säilitamine:**
- Kuivata mõni tund varjus
- Säilita pimedas, jahedas (4–8°C)
- Ära pese enne säilitamist
- Vigastatud mugulad kasuta kohe

### 2025 kogemus

- Istutatud: 18. mai (2,5 vagu, varajased)
- Koristatud: 9. august
- Saak: keskmine
- Pärast: haljasväetis külvatud ✓

### Kartuli haigused

| Haigus | Tunnused | Ennetamine |
|--------|----------|------------|
| **Lehemädanik** | Pruunid laigud lehtedel, levib kiiresti | Ära istuta tomatite kõrvale! Hea õhuring |
| **Kartulimädanik** | Mugulad mädanevad | Korista kuival ilmal, kuivata hästi |
| **Kärnhaigus** | Karedased laigud mugulakanal | pH madal hoida, hea niiskus |

---

## Redis

Redis on üks **kiiremaid** aiakultuure – saak valmib juba 25–30 päevaga! Ideaalne vahekultuure tühjadele kohtadele.

### Miks kasvatada redist

- **Väga kiire:** 25–30 päeva külvist saagini
- **Vahetäitja:** Külva tühjade kohtade vahele (aeglaste kultuuride vahel)
- **Mitu saaki:** Aprillist septembrini saab külvata iga 2–3 nädala järel
- **Lihtne:** Idaneb jahedas, ei vaja ettekasvatust

### Sordid

| Sort | Kuju | Värvus | Valmimisaeg | Märkused |
|------|------|--------|-------------|----------|
| **'Cherry Belle'** ⭐ | Ümar | Punane | 22–25 päeva | Klassika, krõbe |
| **'Jõgeva'** ⭐ | Ümar | Punane | 25–30 päeva | Eesti sort, kindel valik |
| **'French Breakfast'** | Piklik | Punane-valge | 25–28 päeva | Mahe maitse |
| **'Zlata'** | Ümar | Kollane | 30–35 päeva | Erinev, dekoratiivne |
| **'Viola'** | Ümar | Lilla | 30–35 päeva | Dekoratiivne |
| **Taliredis 'Taikon'** | Pikk | Valge | 50–60 päeva | Suur, haljasväetiseks |

### Kasvatamine

| Tegevus | Juhis |
|---------|-------|
| Külviaeg | Aprill–september (iga 2–3 nädala järel) |
| Mullatemperatuur | Min 5°C (optimaalne 10–18°C) |
| Sügavus | 1–2 cm |
| Vahe reas | 3–5 cm |
| Ridade vahe | 10–15 cm |

**Külvi nipid:**
- Külva **hõredalt** – tihedalt kasvanud redisid ei moodusta mugulaid
- Hõrenda 3–5 cm peale, kui liiga tihe
- Külva varjulisemasse kohta (suve keskpaigas)
- Hoia muld **ühtlaselt niiske** – kuivus = kibedam maitse ja pragunemine

### Paigutus aias

```
VAHETÄITJANA (aeglaste kultuuride vahel):
┌─────────────────────────────────────────────┐
│  PORGAND   │ redis │  PORGAND  │ redis │    │
│  (aeglane) │  rrr  │ (aeglane) │  rrr  │    │
│            │       │           │       │    │
│  ← Redis valmib enne, kui porgand ruumi vajab →  │
└─────────────────────────────────────────────┘

ERALDI REANA:
┌─────────────────────────────────────────────┐
│  r r r r r r r r r r r r r r r r r r r r   │
│  ← Külva iga 2 nädala järel uus rida →      │
└─────────────────────────────────────────────┘
```

### Hooldus

| Tegevus | Sagedus | Märkused |
|---------|---------|----------|
| Kastmine | Regulaarselt | Ühtlane niiskus = mahe maitse |
| Hõrendamine | Vajadusel | 3–5 cm vahe mugulaid moodustavatele |
| Rohimine | Alguses | Redis on nõrk konkurent |
| Väetamine | Pole vajalik | Komposti piisab |

### Korjamine

- **Millal:** Kui mugul on 2–3 cm läbimõõduga (tunned läbi mulla)
- **Ära oota liiga kaua** – muutub puiseks ja kibedaks
- Korja hommikul (krõbedam)
- Säilib külmkapis 1–2 nädalat (lehed eemaldatud)

### Probleemid

| Probleem | Põhjus | Lahendus |
|----------|--------|----------|
| Ei moodusta mugulat | Liiga tihe külv | Hõrenda! |
| Kibe maitse | Kuivus, liiga vana | Kasta, korja õigel ajal |
| Pragunenud | Ebaühtlane kastmine | Hoia ühtlaselt niiske |
| Puine | Liiga kaua kasvanud | Korja varem |
| Augud lehtedel | Maakirp | Katteloor, niiske muld |

### Kalender

| Kuu | Tegevus |
|-----|---------|
| Aprill | Esimene külv (niipea kui maa haritav) |
| Aprill–august | Järelkülvid iga 2–3 nädala järel |
| Mai–september | Korjamine 25–30 päeva pärast külvi |
| September | Viimane külv (valmib enne külmi) |

**💡 Nipp:** Redis ja porgand on head naabrid – külva redist porgandirea vahele. Redis valmib enne, kui porgand ruumi vajab, ja märgistab aeglaselt idaneva porgandi rea!

---

## Maapirn (Jerusalem artichoke)

Maapirn (*Helianthus tuberosus*) on **püsikkultuur**, mis annab hiiglaslikku saaki minimaalse vaevaga. Üks kord istutatud, annab saaki aastaid!

### Miks kasvatada maapirni

- **Püsik:** Istuta üks kord, korja igal aastal
- **Hiigelsaak:** 2–3 kg mugulaid taime kohta
- **Hooldusevaba:** Peaaegu ei vaja hooldust
- **Tervislik:** Sisaldab inuliini (prebiootikum, soolestikule hea)
- **Dekoratiivne:** 2–3 m kõrged taimed kollaste õitega (nagu päevalilled)

**⚠️ Hoiatus:** Maapirn levib agressiivselt! Istuta kohta, kust ta levida ei saa, või kasuta tõket.

### Sordid

| Sort | Mugula värv | Kuju | Märkused |
|------|-------------|------|----------|
| **'Fuseau'** ⭐ | Valge | Piklik, sile | Lihtsam puhastada |
| **'Bianka'** | Valge | Ümar | Levinud Eestis |
| **'Rouge'** | Punane | Ümar | Dekoratiivne |
| Tavaline (nimetamata) | Valge | Konarlik | Sageli naabri käest, väga tugev |

**NB:** Enamik Eestis kasvavaid maapirne on "nimetamata" kohalikud vormid – need on väga vastupidavad ja annavad head saaki.

### Kasvatamine

**Istutamine:**

| Tegevus | Juhis |
|---------|-------|
| Aeg | Märts–aprill (niipea kui maa sulanud) |
| Mugulad | Säilitatud eelmise aasta saagist või osta |
| Sügavus | 10–15 cm |
| Vahe | 50–60 cm (taimed on suured!) |
| Koht | Päikeseline või poolvarjuline |

**Koha valik (OLULINE!):**
- Istuta **eraldi alale** – maapirn levib juurte kaudu
- Hea koht: aia serv, tara äär, eraldi nurk
- Või kasuta **juuretõket** (süvistatud plekk, betoon)
- Ära istuta peenrasse – võtab üle!

```
SOBILIK KOHT:
┌─────────────────────────────────────────────┐
│                   AED                        │
│   ┌────────┐                                │
│   │Maapirn │ ← Aia servas, eraldi          │
│   │  🌻🌻🌻  │    nurgas                     │
│   │  🌻🌻🌻  │                               │
│   └────────┘                                │
│        ↑                                    │
│   Tõke (betoon, plekk 40 cm sügavusse)      │
└─────────────────────────────────────────────┘
```

### Hooldus

| Tegevus | Sagedus | Märkused |
|---------|---------|----------|
| Kastmine | Harva | Väga põuakindel |
| Väetamine | Pole vajalik | Kasvab vaeseseski mullas |
| Tugi | Vajadusel | Väga tuulises kohas |
| Levik | Jälgi! | Eemalda levivad taimed |

**Praktiliselt hooldusevaba!** Maapirn kasvab isegi seal, kus muud ei kasva.

### Korjamine

**Millal korjata:**
- **Sügis (oktoober–november):** Pärast esimesi külmi (magusam maitse!)
- **Talvel:** Korja mullas jooksvalt, vastavalt vajadusele
- **Kevad:** Enne uue kasvu algust (märts)

**Kuidas korjata:**
- Kaeva hanguga või labidaga
- Korja kõik mugulad, mida näed (väiksemad jäta tagasi)
- Hoia mõned mugulad mullas – järgmise aasta taimed

**⚠️ Oluline:** Iga jäänud mugulatükk kasvatab uue taime – seepärast ta nii hästi levibki!

### Säilitamine

| Viis | Kestus | Märkused |
|------|--------|----------|
| Mullas (parim!) | Terve talv | Korja vastavalt vajadusele |
| Külmkapis | 2–3 nädalat | Niiske liiv/turvas |
| Keldris | 1–2 kuud | Liivas, jahe ja niiske |
| Külmutamine | 6+ kuud | Koori, blanšeeri, külmuta |

**💡 Nipp:** Parim "säilituskoht" on maa – jäta mugulad talveks mulda ja kaeva välja siis, kui vaja!

### Kasutamine köögis

| Viis | Kirjeldus |
|------|-----------|
| **Toores** | Krõbe, pähklise maitsega – salatites |
| **Küpsetatud** | Nagu kartul, aga magusam |
| **Praetud** | Võiga pannil, karamelliseerub |
| **Supis/pürees** | Kreemjas tekstuur |
| **Chips** | Õhukesed viilud, praetud |

**⚠️ Seedimise hoiatus:** Inuliin võib alguses põhjustada kõhugaase. Alusta väikeste kogustega!

### Maapirni kalender

| Kuu | Tegevus |
|-----|---------|
| Märts–aprill | **Istutamine** (mugulad 10–15 cm sügavusele) |
| Mai–september | Kasvab iseseisvalt, peaaegu ei vaja hooldust |
| September | Õitsemine (kollased, päevalillelaadsed) |
| Oktoober–november | **Korjamise algus** (pärast esimesi külmi) |
| November–märts | Korja mullas vastavalt vajadusele |
| Märts | Viimane korjamine enne kevadist kasvu |

### 2026 plaan

- **Koht:** Eraldi ala aia servas (mitte peenras!)
- **Kogus:** 5–10 mugulat alustuseks (annab juba esimesel aastal head saaki)
- **Variant:** Küsi naabrilt – levib nii hästi, et sageli antakse tasuta!

---

## Peenar 4: Kurgid + Kõrvitsalised

2025 olid siin kartulid → 2026 on hea koht kõrvitsalistele (eri sugukond, hea rotatsioon).

### Kurgid

→ **Vaata detailset juhendit:** [kurgid/kurkide-hooldus.md](../kurgid/kurkide-hooldus.md)

**Testitud 2025:** Andrus ⭐, Alex ⭐ (esimene saak 30. juuli)

**2026 sordid (avamaa):**
- **Adam F1** ⭐ – populaarseim Eestis, iseviljuv, mõruvaba
- **Alex F1** ⭐ – väga varajane, haiguskindel

### Kultuurid

| Kultuur | Kogus | Vahe | Saak algab | Märkused |
|---------|-------|------|------------|----------|
| **Avamaakurk** | 4–5 taime | 60–80 cm | Juuli lõpp | Vt [kurkide-hooldus.md](../kurgid/kurkide-hooldus.md) |
| **Suvikõrvits** | 2–3 taime | 80–100 cm | August | 'Black Beauty' ⭐ |
| **Patisson** | 2 taime | 80–100 cm | August | 'Disco' ⭐ |
| Till | Puistatud | – | – | Kurgi vahele, meelitab tolmeldajaid |
| Päevalill | 4 tk | 50 cm | – | Peenra otstes, tuulekaitse |

→ Kõrvitsaliste detailne info: vt **Kõrvitsalised** sektsioon allpool

### Paigutus peenras (1 × 9 m)

```
┌────┬─────────────────┬─────────────────────────────────────┬────┐
│Päev│    KURGID       │     SUVIKÕRVITS + PATISSON          │Päev│
│lill│   ●   ●   ●     │     ◆     ◆     ◆     ◇     ◇       │lill│
│    │   kurk  kurk    │   suvi-  suvi-  suvi-  pati- pati-  │    │
│    │     + till      │   kõrv.  kõrv.  kõrv.  sson  sson   │    │
│    │    vahele       │                                      │    │
│    │   ← 3 m →       │           ← 5,5 m →                  │    │
└────┴─────────────────┴─────────────────────────────────────┴────┘
 25cm                                                          25cm

●  = kurk (60–80 cm vahe)
◆  = suvikõrvits (80–100 cm vahe)
◇  = patisson (80–100 cm vahe)
```

### Hoolduskalender

| Kuu | Tegevus |
|-----|---------|
| Mai algus | Kurgi ja kõrvitsaliste **ettekasvatamine** (3–4 nädalat) |
| Mai | Tilli esimene külv (otse peenrasse) |
| Juuni algus | **Istutamine õue** (muld min 15°C, öökülmad möödas) |
| Juuni | Multšimine heinaga |
| Juuni–august | Regulaarne kastmine (mulda, mitte lehtedele!) |
| Juuli lõpp | Kurgi saagi algus |
| August | Suvikõrvitsa ja patissoni saagi algus – **korja noorelt!** |
| August–sept | Regulaarne korjamine (iga 2–3 päeva) |
| September | Viimased viljad enne öökülmi |

---

## Porgandi kasvatamine

→ Porgand kasvab 2026 **Peenras 3** (koos sibulalistega)

**Sordid Eestis:**

| Sort | Tüüp | Suurus | Märkused |
|------|------|--------|----------|
| 'Nantes' tüübid | Klassika | 15–20 cm | Silindriline, magus |
| 'Jõgeva Nantes' | Kohalik | 15–18 cm | Hea säilivus |
| 'Autumn King' | Hiline | 20–25 cm | Talveporgand |
| 'Paris Market' | Ümar | 3–5 cm | Kiire, raske mulla jaoks |

**Porgandi külv:**

| Tegevus | Juhis |
|---------|-------|
| Aeg | Aprill keskpaik – mai algus (muld min 7°C) |
| Sügavus | 1–2 cm (väga pinnapealne!) |
| Vahe reas | 3–5 cm (harvendada 5–8 cm peale) |
| Ridade vahe | 20–25 cm |
| Idanemisaeg | 14–21 päeva (aeglane!) |

**Nipid idanemiseks:**
- Leota seemneid 24h enne külvi
- Kata rida kattelooriga (hoiab niisket)
- Eemalda katteloor kohe, kui tõusmed näha
- Või: külva koos redisega (redis märgib rea, korjatakse enne)

**Porgandi säilitamine:**
- Koristamisel lõika lehed 2 cm pealt
- Säilitamine liivas/turbas keldris (0–4°C)
- Või jäta mulda ja kaeva vastavalt vajadusele (multši paksult)

---

## Laugud (Allium)

→ Laugud kasvavad 2026 **Peenras 3** (koos porgandi ja hernega – klassikaline kombinatsioon!)

Laugud on kasulikud kaaslaskultuurid – peletavad kahjureid (eriti porgandikärbest, lehetäisid) ja sobivad paljude köögiviljadega.

**Taksonoomia:** perekond Allium (laugud), sugukond Amaryllidaceae (amarüllilised) / Alliaceae (laugulised)

### Kultuurid

| Kultuur | Tüüp | Istutus/külv | Saak | Märkused |
|---------|------|--------------|------|----------|
| **Sibul (pesasibul)** | Istikud | Aprill | August–sept | Lihtne, kindel saak |
| **Sibul (seemnest)** | Seeme | Märts (ette) | August–sept | Pikem hooaeg |
| **Porru** | Seeme/istik | Märts (ette), mai (õue) | Sept–nov | Vajab pikka hooaega |
| **Murulauk** | Püsik | Kevad/sügis | Kogu hooaeg | Istuta korra, korja aastaid |
| **Talvesibul** | Seeme/istik | August–sept | Järgmine kevad | Ülitalvitub |

### Sibul (pesasibul / komplektsibul)

**Istutamine:**

| Tegevus | Juhis |
|---------|-------|
| Aeg | Aprill (kui maa on haritav) |
| Sügavus | Nina veidi maapinnast väljas |
| Vahe reas | 10–15 cm |
| Ridade vahe | 25–30 cm |

**Hooldus:**
- Rohimine – sibul ei talu konkurentsi
- Kastmine – mõõdukas, kuival perioodil
- Väetamine – kompost piisab, mitte liiga palju lämmastikku

**Koristamine (august–september):**
- Kui lehed on ~50% kollased/lamandunud
- Tõmba välja, kuivata 2 nädalat varjus
- Säilib kuivas jahedas 6+ kuud

### Porru

**Miks kasvatada:**
- Mahe sibulalaadne maitse
- Kõrge külmataluvus – saab korjata hilissügiseni
- Sobib suuppidesse, hautistesse

**Ettekasvatamine:**

| Tegevus | Aeg | Juhis |
|---------|-----|-------|
| Külv | Märts | Külvikasti, 1 cm sügavusele |
| Idanemine | 10–14 päeva | 15–20°C |
| Kasvuaeg sees | 10–12 nädalat | Kuni ~15–20 cm kõrgused |
| Istutamine õue | Mai keskpaik–lõpp | Pärast öökülmade ohtu |

**Istutamine:**
- Tee augud 15 cm sügavused (kepi või sõrmega)
- Lase istik auku, ära täida mullaga
- Kasta auk veega täis (muld settib ise)
- Vahe: 15 cm reas, 30 cm ridade vahel

**Hooldus:**
- Muldamine – tõsta mulda varre ümber (valge osa pikem)
- Kastmine – regulaarne, eriti kuival ajal
- Sügiseni võib peenrasse jätta

**Koristamine:**
- Alates septembrist kuni esimeste tugevate külmadeni
- Või jäta mulda, kaeva vastavalt vajadusele (multši)

### Murulauk

**Püsikkultuur** – istuta üks kord, korja aastaid!

**Istutamine:**

| Tegevus | Juhis |
|---------|-------|
| Aeg | Kevad või sügis |
| Koht | Peenra serv, püsiv koht |
| Vahe | 20–25 cm (taim laieneb) |
| Muld | Tavaline aiamuld, mitte liiga märg |

**Hooldus:**
- Peaaegu hooldusevaba
- Jagage iga 3–4 aasta järel (taim tiheneb)
- Lõika õisi, kui ei taha isekülvi

**Korjamine:**
- Lõika lehti 3–5 cm kõrguselt maapinnast
- Taim kasvab tagasi ~2–3 nädalaga
- Korja terve hooaja (aprill–oktoober)

**Kasutamine:**
- Värskelt: salatid, võileivad, munaroad
- Õied on ka söödavad (dekoratsioon)

### Sibulaliste paigutus aias

```
VARIANT A: Porgandipeenra servas
┌─────────────────────────────────────────────────────────────────┐
│ Murulauk │  PORGAND  │  Sibul (rida)  │  SALATID  │ Murulauk   │
│ (püsiv)  │           │                │           │  (püsiv)   │
└─────────────────────────────────────────────────────────────────┘

VARIANT B: Eraldi ala peenras 4
┌─────────────────────────────────────────────────────────────────┐
│ PORGAND │ SIBUL + PORRU │    SALATID    │ Murulauk │
│         │   (2 m ala)   │               │  (serv)  │
└─────────────────────────────────────────────────────────────────┘
```

**NB:** Laugud + porgand on klassikaline kombinatsioon – sibul peletab porgandikärbest, porgand peletab sibulakärbest!

### Laukude kalender

| Kuu | Tegevus |
|-----|---------|
| Märts | Porru ettekasvatuse algus |
| Aprill | Pesasibulate istutamine, murulaugu istutamine/jagamine |
| Mai | Porru istutamine õue |
| Juuni–august | Murulaugu korjamine, sibulate hooldus |
| August | Talvesibula külv (järgmise aasta saagiks) |
| August–sept | Sibulate koristamine, kuivatamine |
| Sept–nov | Porru koristamine |

### Sordid

**2025 kogemus:** ⚠️ 'Stuttgarter' segusort ei kasvanud hästi – proovi 2026 teist sorti!

| Kultuur | Sordid Eestis | Märkused |
|---------|---------------|----------|
| Pesasibul | **'Sturon'** ⭐, **'Hercules'** ⭐, 'Cupido' | Kõik hea säilivusega, proovi neid! |
| Pesasibul | 'Stuttgarter Riesen' | ⚠️ 2025 ei toiminud, väldi |
| Külvisibul | 'Stuttgarter', 'Ailsa Craig' | Seemnest, pikem hooaeg |
| Porru | 'Hannibal', 'Carentan', 'Musselburgh' | Talvekindlad sordid |
| Murulauk | Tavaline aiamurulauk | Sageli naabri käest |
| Talvesibul | 'Senshyu Yellow' | Sügisel istutamiseks |

---

## Lehtviljad (lehtpeet, lehtkapsas, spinat)

Lehtviljad on suurepärased täiendused aeda – vitamiinirikkad, kiire kasvuga ja paljusid saab korjata pidevalt terve hooaja vältel.

### Lehtpeet (mangold / Swisschard)

**Ladina nimi:** Beta vulgaris var. cicla
**Sugukond:** Rebasabalised (Chenopodiaceae)

Lehtpeet on peedi sugulane, mida kasvatatakse lehtede, mitte juurika pärast. Dekoratiivne ja maitsev!

**Miks kasvatada:**
- Korjatav kogu hooaja (aprill–november)
- Dekoratiivsed värvid (punane, kollane, oranž, valge varred)
- Talub kerget külma (kuni -5°C)
- Lihtne kasvatada, vähe kahjureid
- Üks külv = mitu kuud saaki

#### Sordid ⭐

| Sort | Varre värv | Märkused |
|------|------------|----------|
| **'Bright Lights'** ⭐ | Segu (mitmevärviline) | Populaarne, dekoratiivne, hea maitse |
| **'Rhubarb Chard'** ⭐ | Punane | Rabarberisarnane välimus, magusam |
| **'Fordhook Giant'** | Valge | Klassikaline, suured lehed |
| **'Golden'** | Kollane | Mahedam maitse, ilus värv |
| **'Rainbow'** | Segu | Nagu 'Bright Lights' |

#### Külvamine ja istutamine

| Tegevus | Aeg | Juhis |
|---------|-----|-------|
| **Otsekülv** | Aprill–juuni | 2 cm sügavusele, 5–10 cm vahega |
| **Ettekasvatamine** | Märts–aprill | Külvipotis, istuta mai keskpaigas |
| **Harvendamine** | Kui 5–10 cm kõrgused | Jäta 20–30 cm vahe (harvendatud taimed söö ära!) |

```
Külviskeem:
    20–30 cm        20–30 cm
  ←─────────→     ←─────────→
     ┌───┐           ┌───┐           ┌───┐
     │ L │           │ L │           │ L │
     └───┘           └───┘           └───┘
```

#### Hooldus

- **Kastmine:** Regulaarne, ühtlane niiskus (kuivus = kibedad lehed)
- **Väetamine:** Kompost kevadel, lämmastikurikas väetis soodustab lehekasvu
- **Multšimine:** Hoia muld niiske ja umbrohuvaba
- **Rohimine:** Esimesed nädalad olulised

#### Korjamine

- **Alusta:** Kui lehed ~20–25 cm pikad (6–8 nädalat pärast külvi)
- **Meetod:** Lõika või murra välimisi lehti, jäta südamik kasvama
- **Sagedus:** Iga 1–2 nädala tagant
- **Hooaja pikendamine:** Kata kattelooriga – korjatav kuni novembrini

```
Korjamine:
       ╱ lõika välimisi
      ╱   lehti
     ↓
   ┌─────┐
  ╱│     │╲ ← jäta südamik
 ╱ │  ●  │ ╲   alles
╱  │     │  ╲
   └─────┘
```

**Köögis:**
- **Lehed:** Nagu spinat (sautee, supid, smoothied)
- **Varred:** Eraldi küpsetada (kauem kui lehed), nagu spargel

---

### Lehtkapsas (kale)

**Ladina nimi:** Brassica oleracea var. sabellica
**Sugukond:** Ristõielised (Brassicaceae)

Lehtkapsas on supervili! Vitamiinirikas, külmakindel ja kasvab ka läbi talve. Pärast esimesi külmi muutub maitse magusaks.

**Miks kasvatada:**
- **Talvine rohelus** – korjatav oktoobrist aprillini!
- Äärmiselt vitamiini- ja mineraalirikas (A, C, K, raud, kaltsium)
- Talub külma kuni -15°C (kaetult veelgi rohkem)
- Maitse paraneb pärast külma (tärklis → suhkur)
- Vähe kahjureid sügisel/talvel

#### Sordid ⭐

| Sort | Lehe tüüp | Kõrgus | Külmakindlus | Märkused |
|------|-----------|--------|--------------|----------|
| **'Westlandse Winter'** ⭐ | Kähar, roheline | 60–80 cm | Väga hea (-15°C) | Hollandi sort, talvekindel |
| **'Dwarf Green Curled'** ⭐ | Kähar, roheline | 30–40 cm | Hea | Kompaktne, sobib väiksemasse aeda |
| **'Nero di Toscana'** ⭐ | Sile, tumesinine | 60–90 cm | Keskmine | Itaalia "dinosaurusekapsas", peenmaitse |
| **'Red Russian'** | Hammastatud, punakasvioletne | 50–70 cm | Väga hea | Dekoratiivne, mahedam maitse |
| **'Redbor'** | Kähar, tumepunane | 60–80 cm | Hea | Väga dekoratiivne, intensiivne värv |
| **'Vates'** | Kähar, sinine-roheline | 40–60 cm | Väga hea | Klassikaline Ameerika sort |

#### Külvamine ja istutamine

**Talvise saagi jaoks:**

| Tegevus | Aeg | Juhis |
|---------|-----|-------|
| **Ettekasvatamine** | Juuni–juuli | Potis/külvialuses |
| **Istutamine õue** | Juuli–august | Vahe 40–50 cm |
| **Otsekülv** | Juuni–juuli | 1 cm sügavusele |

**Varase saagi jaoks:**

| Tegevus | Aeg | Juhis |
|---------|-----|-------|
| **Ettekasvatamine** | Märts–aprill | Sees alustada |
| **Istutamine õue** | Mai | Pärast öökülmade ohtu |
| **Otsekülv** | Mai | Kui muld soojenenud |

```
Istutamine (talvesaagiks):
      40–50 cm
   ←───────────→
  ┌───┐       ┌───┐
  │ K │       │ K │    K = lehtkapsas
  └───┘       └───┘

Istutusaeg: juuli–august
Saak: oktoober → aprill (järgmine kevad!)
```

#### Hooldus

- **Kastmine:** Regulaarne, eriti kuival perioodil (suve alguses)
- **Väetamine:** Kompost istutamisel, kesksel kasvuperioodil võib anda lämmastikku
- **Multšimine:** Sügisel paksem multš (kaitse külma eest)
- **Tugipuu:** Kõrged sordid võivad vajada tuge (tuul, lumi)

#### Kahjurid

⚠️ Lehtkapsas kuulub kapsaste sugukonda – samad kahjurid!

| Kahjur | Oht | Tõrje |
|--------|-----|-------|
| **Kapsauss** (kapsaliblikas) | Suvi | Katteloor, Bt-preparaat, käsitsi korjamine |
| **Kapsasääsk** | Suvi | Katteloor |
| **Naeris-maakirp** | Kevad | Katteloor, niiske muld |
| **Lehetäid** | Suvi | Seebivesi, lepatriinud |

**NB:** Sügisel ja talvel on kahjurite surve palju väiksem – seepärast kasvatagi talviseks saagiks!

#### Korjamine

- **Alusta:** Kui lehed ~15–20 cm pikad
- **Meetod:** Võta alumisi välimisi lehti, jäta ladvapung kasvama
- **Regulaarselt:** Korjamine stimuleerib uut kasvu
- **Talvel:** Korja vastavalt vajadusele, taim kasvab aeglaselt edasi

```
Korjamine:
        ╱╲
       ╱  ╲ ← jäta ladvapung!
      ╱    ╲
     ╱──────╲
    ╱│      │╲
      │    │
      ↓    ↓
   korja alumisi
   lehti esmalt
```

**Köögis:**
- **Toores:** Salatites (masseerida õli + soolaga, muutub pehmemaks)
- **Küpsetatult:** Sautee, supid, smoothied, kale chips (krõpsud ahjus)
- **Talvel:** Asendab spinatit ja salateid

#### Lehtkapsase kalender

| Kuu | Tegevus |
|-----|---------|
| Märts–aprill | Varase saagi ettekasvatamine |
| Mai | Varase saagi istutamine |
| Juuni–juuli | **Talvesaagi külv/ettekasvatamine** ⭐ |
| Juuli–august | Talvesaagi istutamine (40–50 cm vahe) |
| Oktoober | Saak algab (pärast esimesi külmi magusam!) |
| November–märts | **Talvine korjamine** – vastavalt vajadusele |
| Aprill | Taim läheb õitsema, saak lõppeb |

---

### Spinat

**Ladina nimi:** Spinacia oleracea
**Sugukond:** Rebasabalised (Chenopodiaceae)

Spinat on kiire saagiga lehtköögivili, mis sobib eriti kevadeks ja sügiseks – suvel kipub õitsema minema.

**Miks kasvatada:**
- Väga kiire saak (30–45 päeva)
- Kaks hooaega: kevad + sügis
- Rauarikas, vitamiinirikas
- Talub kerget külma

#### Sordid

| Sort | Tüüp | Märkused |
|------|------|----------|
| **'Matador'** ⭐ | Sile leht | Aeglane õitsemaminekuga, kevadeks |
| **'Bloomsdale Long Standing'** | Kähar leht | Kuumus-tolerantne, aeglane õitsema |
| **'Giant Winter'** | Sile leht | Talvekülviks, külmakindel |
| **'Palco F1'** | Sile leht | Kiire kasv, levinud |

#### Külvamine

| Hooaeg | Külviaeg | Saak | Märkus |
|--------|----------|------|--------|
| **Kevad** | Märts–aprill | Mai–juuni | Külva kohe, kui muld haritav |
| **Sügis** | August–september | September–november | Pärast kuumust, jahedamas |

- Külvisügavus: 2 cm
- Vahe reas: 5–10 cm (harvendada 15 cm peale)
- Ridadevahe: 25–30 cm
- Järelkülv: iga 2–3 nädala tagant (pidev saak)

#### Hooldus

- **Kastmine:** Ühtlane niiskus (kuivus → õitsemine!)
- **Varjutamine:** Suvel osaliselt varjus (kuumus → õitsemine)
- **Rohimine:** Hoida umbrohuvaba
- **Väetamine:** Kompost piisab, mitte liiga palju

#### Probleemid

| Probleem | Põhjus | Lahendus |
|----------|--------|----------|
| **Õitseminek** (bolting) | Kuumus, pikk päev, kuivus | Sügiskülv, kastmine, sordivalik |
| **Kollased lehed** | Lämmastikupuudus | Kompost, vedel lämmastikväetis |
| **Lehetäid** | Suvi | Seebivesi, katteloor |

**NB:** Spinat ei sobi suviseks saagiks Eesti pikal päeval – kasuta suve alguses hoopis lehtpeeti!

#### Korjamine

- **Alusta:** Kui lehed 8–10 cm pikad (baby spinat) või 15+ cm (täiskasvanud)
- **Meetod 1:** Lõika välimisi lehti, jäta südamik (pikem saak)
- **Meetod 2:** Lõika kogu taim 3 cm kõrguselt (kasvab tagasi 1×)
- **Säilitamine:** Külmkapis niiskes kotis 5–7 päeva; külmuta blanšeeritult

---

### Lehtviljad peenras 3

Lehtviljad sobivad suurepäraselt **Peenrasse 3** järelkülviks pärast herneid (juulist):

```
PEENAR 3 – SUVI/SÜGIS (pärast herneid)
┌─────────────────────────────────────────────────────────────────┐
│ Spinat │ Lehtpeet │  (porru jääb) │ Lehtkapsas │ Salatid │
│ (külv  │ (juuni   │               │ (juuli     │ (külv   │
│ aug)   │  külv)   │               │  istutus)  │  juuli) │
└─────────────────────────────────────────────────────────────────┘
         ↑                                        ↑
    herneste alale                         herneste alale
```

### Lehtviljad – võrdlustabel

| Omadus | Lehtpeet | Lehtkapsas | Spinat |
|--------|----------|------------|--------|
| **Saak** | Mai–november | Oktoober–aprill | Mai–juuni, sept–nov |
| **Korjamismeetod** | Välimised lehed | Välimised lehed | Kogu taim/välimised |
| **Kuumataluvus** | Hea | Keskmine | Halb (õitseb) |
| **Külmataluvus** | Kuni -5°C | Kuni -15°C | Kuni -5°C |
| **Talvine saak** | Ei (kattelooriga kuni nov) | **JAH** ⭐ | Ei |
| **Raskusaste** | Lihtne | Lihtne | Keskmine |
| **Dekoratiivsus** | Kõrge (värvid) | Keskmine–kõrge | Madal |

---

## Maitsetaimed (ürdid)

→ **Vaata detailset juhendit:** [maitsetaimed/maitsetaimede-hooldus.md](../maitsetaimed/maitsetaimede-hooldus.md)

Maitsetaimed on kasulikud kaaslaskultuurid – meelitavad tolmeldajaid, peletavad kahjureid ja maitsestavad toitu!

### Paigutus 2026

| Maitsetaim | Koht | Märkused |
|------------|------|----------|
| **Basiilikus** | P2: tomatite vahel | 4–6 taime |
| **Till** | P4: kurkide vahel | Puista laiali |
| **Petersell** | P3: servad | 4–6 taime |
| **Koriander** | Servad | Külva järelkülve |
| **Püsikud (ürdinurk)** | Eraldi ala | Pune, tüümian, salvei, leeskputk |

### Maitsetaimede tüübid

| Tüüp | Maitsetaimed | Kasvukoht |
|------|--------------|-----------|
| Üheaastased | Basiilikus, till, koriander | Peenras kaaslastena |
| Kaheaastased | Petersell | Peenra servad |
| Püsikud ⭐ | Pune, tüümian, salvei, leeskputk, murulauk | Eraldi ürdinurk |

---

## Kõrvitsalised (Cucurbitaceae)

Kõrvitsalised on soojust armastavad kultuurid, mis vajavad pikka kasvuperioodi ja palju ruumi.

### Kultuurid

| Kultuur | Tüüp | Ruumivajadus | Saak algab | Märkused |
|---------|------|--------------|------------|----------|
| **Suvikõrvits** | Üheaastane | 1 m² taime kohta | 50–60 päeva | Korja noorelt! |
| **Patisson** | Üheaastane | 1 m² taime kohta | 50–60 päeva | Dekoratiivne, mahe maitse |
| **Arbuus** | Üheaastane | 2–3 m² taime kohta | 80–100 päeva | **→ Kasvuhoonesse!** Vt kasvuhoone-plaan.md |
| **Talvkõrvits** | Üheaastane | 2–3 m² taime kohta | 90–120 päeva | Säilib talvel |
| **Kurk** | Üheaastane | 0,5–1 m² taime kohta | 50–70 päeva | Vt Peenar 4 sektsioon |

### Testitud sordid 2025

| Kultuur | Sort | Tulemus | Märkused |
|---------|------|---------|----------|
| **Suvikõrvits** | 'Black Beauty' | ⭐ Hea | Klassikaline roheline suvikõrvits |
| **Patisson** | 'Disco' | ⭐ Hea | Kollane, mahe maitse |
| **Arbuus** | 'Sugar Baby' | → Kasvuhoonesse 2026 | Avamaal liiga riskantne Eestis |

### Kasvatamine

**Ettekasvatamine:**

| Tegevus | Aeg | Juhis |
|---------|-----|-------|
| Külv | Mai algus | Otse suuremasse potti (3–4 nädalat enne õue) |
| Idanemine | 5–10 päeva | 22–28°C, niiske muld |
| Istutamine õue | Juuni algus | Pärast öökülmade ohtu, soe muld |

**NB:** Kõrvitsalised EI TALU ümberistutamist! Külva otse suurde potti ja istuta potiga koos.

**Istutamine:**

| Tegevus | Juhis |
|---------|-------|
| Aeg | Juuni algus (muld min 15°C) |
| Koht | Päikeseline, soojapeegeldav (kivi, sein) |
| Vahe | Suvikõrvits/patisson: 80–100 cm; arbuus/talvkõrvits: 150–200 cm |
| Muld | Rikkas, kompostrikas muld |

### Hooldus

| Tegevus | Juhis |
|---------|-------|
| Kastmine | Regulaarne, mulda (mitte lehtedele!) |
| Väetamine | Komposti lisamine, nõgesevesi iga 2 nädala järel |
| Multšimine | Hein/põhk viljade alla (puhtus, kuivus) |
| Tolmlemine | Eesti kliimas tavaliselt putukad toimetulevad |

### Korjamine

**Suvikõrvits ja patisson:**
- Korja NOORELT (15–20 cm suvikõrvits, 8–10 cm patisson)
- Mida rohkem korjad, seda rohkem taim toodab
- Korja iga 2–3 päeva
- Ülevalminud viljad on kibedad ja seemnetega

**Talvkõrvits:**
- Korja enne esimesi külmi
- Vars peab olema kuiv ja kõva
- Jäta 5–10 cm vars külge
- Kuivata 2 nädalat soojas (toatemperatuur)
- Säilib kuni 6 kuud jahedas kuivas kohas

**Arbuus:**
- Küps, kui "kõlksatab" (koputades tume heli)
- Maas olev laik muutub kollakaks
- Vars hakkab kuivama

### Kõrvitsaliste kalender

| Kuu | Tegevus |
|-----|---------|
| Mai algus | Ettekasvatamine (3–4 nädalat) |
| Juuni algus | Istutamine õue |
| Juuni–juuli | Hooldus, kastmine, väetamine |
| Juuli–september | Suvikõrvitsa ja patissoni korjamine |
| September | Talvkõrvitsa ja arbuusi korjamine (enne külmi) |

### 2025 kogemus

- Suvikõrvits 'Black Beauty' ja patisson 'Disco': esimene saak ~9. august
- Arbuus 'Sugar Baby': proovitud

### Kõrvitsaliste haigused

| Haigus | Tunnused | Ennetamine/tõrje |
|--------|----------|------------------|
| **Jahukaste** | Valge kate lehtedel | Hea õhuring, ära kasta lehti |
| **Viirushaigused** | Moonutatud lehed, kirju muster | Eemalda kahjurid (lehetäid), haigeid taimi |
| **Mädanikud** | Viljad mädanevad | Multš viljade alla, hea drenaaž |

---

## Ettekasvatamise kalender

### Veebruar

| Nädal | Kultuur | Tegevus |
|-------|---------|---------|
| 3.–4. nädal | Tšilli | Külv ettekasvatuskasti, 22–25°C |

### Märts

| Nädal | Kultuur | Tegevus |
|-------|---------|---------|
| 1. nädal | **Kartul 'Solist'** | **Eelidandamine** (turbamullas, valge, <20°C) |
| 1.–2. nädal | **Porru** | Ettekasvatuse algus (külvikasti) |
| 2.–3. nädal | Tomat | Külv, 20–22°C |
| 2.–3. nädal | Füüsal | Külv (nagu tomat) |
| 3. nädal | Kartul | Tõsta jahedamasse, lase juurduda |
| 4. nädal | Tšilli | Ümber potimine 9 cm pottidesse |

### Aprill

| Nädal | Kultuur | Tegevus |
|-------|---------|---------|
| 1.–2. nädal | Herned | **Otse peenrasse** (kui maa sulanud) |
| 1.–2. nädal | Spinat | Otse peenrasse |
| 1.–2. nädal | Redis | Otse peenrasse |
| 1.–2. nädal | **Pesasibul** | **Istutamine** (istikud mulda) |
| 1.–2. nädal | **Murulauk** | Istutamine/jagamine (püsik) |
| 1.–2. nädal | **Pune** | Istutamine/jagamine (püsik) |
| 2.–3. nädal | **Porgand** | **Otse peenrasse** (muld min 7°C) |
| 2.–3. nädal | **Basiilikus** | Ettekasvatamine (soe, 20–22°C) |
| 2.–3. nädal | **Petersell** | Ettekasvatamine (või mai otse) |
| 2.–3. nädal | Tomat, füüsal | Ümber potimine suurematesse |
| 2.–3. nädal | Salat | Esimene külv (otse või ettekasv.) |
| 3. nädal | **Kartul** | Maa ettevalmistamine (must kile + katteloor) |
| 3.–4. nädal | Päevalill | Ettekasvatamine (2–3 seemet potti) |
| 4. nädal | **Kartul 'Solist'** | **Mahapanék** (10 cm, 30 cm vahe) |

### Mai

| Nädal | Kultuur | Tegevus |
|-------|---------|---------|
| 1. nädal | Kurk | Ettekasvatamine (3–4 nädalat enne õue) |
| 1. nädal | Till | Esimene külv otse peenrasse |
| 1. nädal | Porgand | Teine külv (hilisem saak) |
| 2. nädal | Salat | Teine külv |
| 2.–3. nädal | **Aedoad** | **Otse peenrasse** (muld min 12°C, öökülmad möödas) |
| 2.–3. nädal | **Porru** | **Istutamine õue** (15 cm sügavusse) |
| 2.–3. nädal | Kõik istikud | **Karastamine** (päevaks õue, ööks sisse) |
| 3.–4. nädal | Päevalill | Istutamine õue |

### Juuni

| Nädal | Kultuur | Tegevus |
|-------|---------|---------|
| 1. nädal | Tomat, tšilli, füüsal | **Istutamine õue** (pärast öökülmi) |
| 1. nädal | **Basiilikus** | **Istutamine õue** (tomatite vahele) |
| 1.–2. nädal | Kurk | **Istutamine õue** (soe muld, min 15°C) |
| 1. nädal | Aedoad | Toe paigaldamine ronivatele |
| 2. nädal | Till | Teine külv |
| 2.–3. nädal | Aedoad (madal) | Võimalik järelkülv (saak augustis) |
| 4. nädal | **Kartul 'Solist'** | **Saak jaanipäevaks!** 🥔 |

---

## Ettekasvatamise tingimused

| Kultuur | Idanemistemperatuur | Kasvutemperatuur | Valgus | Idanemisaeg |
|---------|---------------------|------------------|--------|-------------|
| Tomat | 20–25°C | 18–22°C | Palju | 7–14 päeva |
| Tšilli | 22–28°C | 20–25°C | Palju | 14–21 päeva |
| Füüsal | 20–25°C | 18–22°C | Palju | 10–14 päeva |
| Kurk | 22–28°C | 20–25°C | Palju | 5–7 päeva |
| Päevalill | 18–22°C | 15–20°C | Palju | 7–14 päeva |
| Salat | 15–20°C | 15–18°C | Keskmine | 5–10 päeva |

### Ettekasvatamiseks vajalik

| Vahend | Milleks |
|--------|---------|
| Külvikastid / potid | Tomatid, tšilli, füüsal |
| 9 cm potid | Ümber potimine |
| Külvimurd / turvas | Kerge, toitainevaene segu |
| Taimimuld | Suurematesse pottidesse |
| Kasvulamp (soovitav) | Kui aknalauda valgust vähe |
| Kuumusmatt (soovitav) | Tšillile ja kurgile idanemisel |

---

## Kaevamisvaba meetodi põhimõtted

### Peenra ettevalmistamine kevadel

1. **Ära kaeva** – lisa kompost peale
2. **Komposti kiht:** 5–10 cm küpset komposti
3. **Lase "küpseda":** 2–3 nädalat enne istutamist
4. **Istuta läbi komposti** otse mulda all

### Multšimine

| Millal | Kuidas |
|--------|--------|
| Pärast istutamist | Oota, kuni taimed on kosunud (2–3 nädalat) |
| Materjal | Kuiv hein, põhk, hakitud lehed |
| Paksus | 5–10 cm |
| Ettevaatust | Jäta taimede tüve ümbrus vabaks |

### Toitmine hooaja jooksul

| Kultuur | Vajadus | Väetamine |
|---------|---------|-----------|
| Tomatid, kurgid, tšilli | Kõrge | Nõgesevesi / sõnnikuvesi iga 2 nädala järel juulist |
| Füüsal | Keskmine | Harvem kui tomatid |
| Herned | Madal | Ei vaja (seovad ise lämmastikku) |
| Salatid | Madal–keskmine | Kompost piisab |

### Nõgesevee retsept

1. Pool ämbrit nõgeseid (ilma seemneteta)
2. Täida veega
3. Kata ja lase kääritada 2 nädalat
4. Sega iga paari päeva tagant
5. Lahjenda 1:10 ja kasta taimedele

---

## Külvikord (rotatsioon)

### Miks on vaja?

- Erinevad kultuurid ammutavad eri toitaineid
- Haigused ja kahjurid ei kogune
- Muld jääb tasakaalu

### 4 aasta rotatsiooniplaan

| Aasta | Peenar 1 | Peenar 2 | Peenar 3 | Peenar 4 |
|-------|----------|----------|----------|----------|
| **2025** | Maasikad | Herned + oad | Kurgid + suvikõrvits | Kartul |
| 2026 | Maasikad | Maavitsalised | Herned + oad | Kurgid + kõrvitsalised |
| 2027 | Maasikad | Salatid + porgand | Maavitsalised | Herned + oad |
| 2028 | Maasikad | Kurgid + kõrvitsalised | Salatid + porgand | Maavitsalised |
| 2029 | **Uuenda maasikad** | Herned + oad | Kurgid + kõrvitsalised | Salatid + porgand |

**Rotatsiooni loogika:**
- Liblikõielised (herned, oad) → Maavitsalised (saavad lämmastikku)
- Maavitsalised → Salatid + juurvili (kergemad kultuurid)
- Salatid → Kõrvitsalised (vajavad rikast mulda)
- Kõrvitsalised → Liblikõielised (taastavad mulda)

**NB:** Maasikad on paigas 4–5 aastat (istutatud 2024), siis uuendada uues kohas.

### Rotatsioonigrupid

| Grupp | Kultuurid | Järgneb hästi | 2026 peenar |
|-------|-----------|---------------|-------------|
| Maavitsalised | Tomat, tšilli, füüsal | Pärast herneid/ube (lämmastik!) | Peenar 2 |
| Liblikõielised + juurviljad | Herned, oad, porgand, sibul | Pärast kõrvitsalisi | Peenar 3 |
| Kõrvitsalised | Kurk, suvikõrvits, patisson | Pärast kartuleid | Peenar 4 |
| Lehelised | Salatid, spinat | Järelkülv pärast herneid | Peenar 3 (juulist) |

---

## Kaaslaskultuuride kokkuvõte

### Head naabrid

| Kultuur | Head kaaslased |
|---------|----------------|
| Tomat | Basiilikus, porgand, petersell, peiulill, sibul |
| Tšilli | Basiilikus, tomat, porgand |
| Kurk | Herned, oad, till, salat, päevalill, redis |
| Herned | Porgand, redis, salat, kurk, aedoad |
| **Aedoad** | Kurk, porgand, salat, redis, tomat, herned |
| **Porgand** | Sibul, murulauk, herned, salat, tomat, redis |
| Salat | Porgand, redis, sibul, maasikad, herned |

### Halvad naabrid

| Kultuur | Halvad kaaslased | Põhjus |
|---------|------------------|--------|
| Tomat | Kartul, kapsas, kurk, apteegitill | Haigused, konkurents |
| Kurk | Kartul, salvei, aromaatsed ürdid | Maitse, kasv |
| Herned | Sibul, küüslauk, murulauk | Pärsivad kasvu |
| **Aedoad** | Sibul, küüslauk, murulauk, apteegitill | Pärsivad kasvu |
| **Porgand** | Till (väidetavalt), seller | Konkurents, maitse |
| Salat | – | Sobib peaaegu kõigega |

---

## 2026 kevadine tegevuskava

### Veebruar

- [ ] Tšilli seemnete külv (3.–4. nädal)
- [ ] Ettekasvatuse varustuse kontroll (potid, muld, lambid)

### Märts

- [ ] **Kartuli 'Solist' eelidandamine** (1. nädal) – turbamullas, valge, <20°C
- [ ] **Porru ettekasvatuse algus** (1.–2. nädal)
- [ ] Tomatite ja füüsale külv (2.–3. nädal)
- [ ] Kartul jahedamasse, lase juurduda (3. nädal)
- [ ] Tšilli ümberpotimine (4. nädal)
- [ ] Seemnete hankimine (kui puudu)
- [ ] Sibulaistikute hankimine
- [ ] Kartuli mugulate hankimine ('Solist')

### Aprill

- [ ] Herneste külv otse peenrasse (1.–2. nädal) → P3
- [ ] **Porgandi külv** otse peenrasse (2.–3. nädal) → P3
- [ ] **Pesasibulate istutamine** (1.–2. nädal) → P3
- [ ] Kartuli maa ettevalmistamine: must kile + katteloor (3. nädal)
- [ ] **Kartuli 'Solist' mahapanék** (4. nädal / mai algus) – 10 cm, 30 cm vahe
- [ ] **Murulaugu istutamine/jagamine** (1.–2. nädal) → P3 servad
- [ ] Spinati ja redise külv (1.–2. nädal) → P3
- [ ] **Basiiliku ettekasvatamine** (2.–3. nädal) → P2 jaoks
- [ ] **Peterselli ettekasvatamine** (2.–3. nädal)
- [ ] **Pune istutamine/jagamine** (1.–2. nädal) → serv
- [ ] Tomatite ja füüsale ümberpotimine → P2 jaoks
- [ ] Salatite esimene külv → P3
- [ ] Päevalillede ettekasvatamine → P4 jaoks
- [ ] Komposti lisamine peenardele (kui pole talvel lisatud)

### Mai

- [ ] Kurgi ja **kõrvitsaliste ettekasvatamine** (1. nädal) → P4
- [ ] Tilli esimene külv (otse) → P4
- [ ] Porgandi teine külv (hilisem saak) → P3
- [ ] **Aedubade külv** otse peenrasse (2.–3. nädal) → P3
- [ ] **Porru istutamine õue** (2.–3. nädal) → P3
- [ ] Salatite teine külv
- [ ] Istikute karastamine (2.–3. nädal)
- [ ] Päevalillede istutamine → P4
- [ ] Peenarde multšimine (kus taimed istutatud)

### Juuni

- [ ] Tomatite, tšilli, füüsale istutamine (1. nädal, pärast öökülmi) → P2
- [ ] **Basiiliku istutamine** tomatite vahele (1. nädal) → P2
- [ ] Kurgi ja **kõrvitsaliste istutamine** (1.–2. nädal) → P4
- [ ] Tugede paigaldamine (tomatid, herned, ronivad oad) → P2, P3
- [ ] Tilli teine külv → P4
- [ ] Herneste saagi algus → P3
- [ ] Madalate ubade võimalik järelkülv → P3
- [ ] **Kartul 'Solist' saak jaanipäevaks!** 🥔 (4. nädal)

### Juuli

- [ ] Tomatite kõrvalvõsude eemaldamine (regulaarselt!) → P2
- [ ] Tomatite ladvade näpistamine (kuu lõpus) → P2
- [ ] Herneste saagi lõpp, **jäta juured mulda** → P3
- [ ] **Salatite külv herneste asemele** → P3
- [ ] Küüslaukude koristamine (kui lehed ~50% kollased) → P1
- [ ] Kastmine kuival perioodil (eriti kurgid, tomatid) → P2, P4
- [ ] Kurgi saagi algus → P4

### August

- [ ] Sibulate koristamine ja kuivatamine → P3
- [ ] Suvikõrvitsa ja patissoni korjamine (regulaarselt!) → P4
- [ ] Talvesibula külv (järgmise aasta saagiks)
- [ ] Sügissalatite ja spinati külv → P3 (herneste asemele)
- [ ] Tomatite jätkuv korjamine (ka rohelised → toas valmima) → P2
- [ ] Ubade regulaarne korjamine → P3
- [ ] Maasikate puhastamine ja harvendamine → P1
- [ ] Kurgi korjamine jätkub → P4

### September

- [ ] Tomatite lõplik koristamine (enne öökülmi) → P2
- [ ] Tšilli ja füüsale koristamine → P2
- [ ] Basiiliku koristamine enne külmi → P2
- [ ] Porru koristamise algus → P3
- [ ] Porgandite koristamine → P3
- [ ] Viimased kurgid ja kõrvitsad enne öökülmi → P4
- [ ] Komposti valmistamine (saagikoristuse jäägid)
- [ ] Peenarde puhastamine

### Oktoober

- [ ] Viimaste viljade koristamine
- [ ] Porru koristamine jätkub
- [ ] Haljasväetise kasvamine / niitmine
- [ ] Peenarde ettevalmistamine talveks
- [ ] Komposti lisamine (kui on)
- [ ] Multšimine talveks (kui ei ole haljasväetist)
- [ ] Talvesibulate istutamine (kui ei tehtud augustis)
- [ ] Pugulase pesade eemaldamine

### November

- [ ] Viimaste külmakindlate kultuuride koristamine (porru, kapsas)
- [ ] Peenarde lõplik multšimine
- [ ] Tööriistade puhastamine ja hoiustamine
- [ ] Hooaja kokkuvõtte tegemine (mis toimis, mis mitte)
- [ ] Järgmise aasta plaani eskiis
- [ ] Seemnete tellimine varakult (populaarsed sordid lõppevad!)

---

## Seemnete hankimise nimekiri

| Kultuur | Sort (soovituslik Eestile) | Kogus | Peenar |
|---------|---------------------------|-------|--------|
| **P2: Maavitsalised** ||||
| Tomat | **'Malle'** ⭐, **'Pille'** ⭐ (osta!), 'Maike' (olemas) | 2–3 pakki | P2 |
| Tomat (olemas) | 'Pokusa', 'Moneymaker', 'S. Pierre', 'Zuckertraube' | – | P2 |
| Tomat (proovida) | 'San Marzano 2' (ploomtomat), 'Ananass' (suur) | 1–2 pakki | P2 |
| Tšilli | **'Jalapeño'** ⭐ | 1 pakk | P2 |
| Füüsal | Ananassfüüsal / maafüüsal | 1 pakk | P2 |
| Basiilikus | 'Genovese', 'Thai', 'Lemon' | 1–2 pakki | P2 |
| **P3: Herned + juurviljad** ||||
| Herned | **'Looming'** ⭐, 'Kelvedon Wonder' ⭐, 'Ambassador' | 50–100 g | P3 |
| Aedoad (roniv) | **'Blauhilde'** ⭐ (lilla), **'Neckargold'** ⭐ (kollane) | 50–100 g | P3 |
| Aedoad (madal) | **'Saxa'** ⭐, 'Delinel', 'Purple Teepee' | 50–100 g | P3 |
| Põlduba | **'Jõgeva'** ⭐ (Eesti sort, kindel valik) | 100–200 g | P3 |
| Porgand | 'Naomi', 'Jerada F1' (testitud 2025) | 2–3 pakki | P3 |
| Pesasibul | **'Sturon'** ⭐ või **'Hercules'** (proovida!) | 0,5–1 kg istikuid | P3 |
| Porru | 'Hannibal', 'Carentan' | 1–2 pakki | P3 |
| Murulauk | Tavaline aiamurulauk | Istik/taim | P3 serv |
| Salat | Segu (lehtsalat, lollo, tammeleheline) | 2–3 pakki | P3 järelkülv |
| Spinat | 'Matador', 'Bloomsdale' | 1 pakk | P3 järelkülv |
| Redis | 'Jõgeva', 'Cherry Belle' | 1 pakk | P3 järelkülv |
| **P4: Kurgid + kõrvitsalised** ||||
| Kurk (avamaa) | 'Andrus', 'Alex' (testitud 2025) | 1 pakk | P4 |
| **Suvikõrvits** | **'Black Beauty'** ⭐ (testitud 2025) | 1 pakk | P4 |
| **Patisson** | **'Disco'** ⭐ (testitud 2025) | 1 pakk | P4 |
| Till | Tavaline aiatill | 1 pakk | P4 |
| Päevalill | Kõrge sort ('Russian Giant' vms) | 1 pakk | P4 |
| **Kasvuhoone** ||||
| Kurk (kasvuhoone) | **'Suyu Long'** ⭐ (30 cm, maitsev!) | 1 pakk | KH |
| Tomat (kasvuhoone) | 'Malle', 'Pille', 'San Marzano 2' | (samad kui P2) | KH |
| **Eraldi ala** ||||
| Kartul (varajane) | **'Solist'** ⭐ (jaanipäevaks!) | 2–3 kg mugulaid | eraldi ala |
| Arbuus | **'Sugar Baby'** (kasvuhoonesse!) | 1 pakk | KH |
| **Maapirn** | 'Fuseau' ⭐ või kohalik sort (naabrilt?) | 5–10 mugulat | eraldi ala |
| **Muu / ürdid** ||||
| Petersell | Lehpetersell, kähar | 1–2 pakki | servad |
| Koriander | Tavaline | 1 pakk | servad |
| Till | Tavaline aiatill | 1 pakk | P4 (kurkide vahel) |
| **Ürdinurk (püsikud)** ||||
| Pune | Tavaline aedpune | Istik/taim | ürdinurk |
| Tüümian | Tavaline tüümian | Istik/taim | ürdinurk |
| Salvei | *Salvia officinalis* | Istik/taim | ürdinurk |
| Leeskputk | Tavaline | Istik/taim (naabrilt?) | ürdinurk |

---

## Kahjurid ja haigused

Ennetamine on parim tõrje! Terved taimed, hea õhuring ja õige kastmine hoiavad enamiku probleeme ära.

### Levinumad kahjurid

| Kahjur | Toidutaimed | Tunnused | Tõrje |
|--------|-------------|----------|-------|
| **Lehetäid** | Kõik, eriti herned, oad, kapsad | Kleepuvad, keerdus lehed, sipelgad | Tugev veejuga, seebivesi, neebiõli |
| **Naeris-maakirp** | Ristõielised (redis, kapsas, raps) | Väikesed augud lehtedel, hüppavad mardikad | Katteloor, niiske muld, püretriinisprei |
| **Kapsauss** (kapsaliblikas) | Kapsad, lillkapsas, brokoli | Suured augud, rohelised röövikud | Käsitsi korjamine, Bacillus thuringiensis |
| **Porgandikärbes** | Porgand, seller, petersell | Punakad lehed, ussikäigud juurvilja sees | Sibula kaaslaskultuur, katteloor |
| **Sibulakärbes** | Sibul, porru, küüslauk | Närbuvad taimed, tõugud juurtes | Porgandi kaaslaskultuur, vahetuvkülv |
| **Kartulimardikad** | Kartul, tomat, baklažaan | Triibulised mardikad, oranžid munad | Käsitsi korjamine, Bacillus thuringiensis |
| **Nälkjad** | Kõik, eriti salatid, kapsad | Limased jäljed, augud lehtedel | Õllelõksud, tuhk/liiv ümber, nälkjatõrje graanulid |
| **Vaarikamardikas** | Vaarikas (vt vaarikate hooldus) | "Ussitanud" marjad | Raputa hommikul ajalehele |
| **Lestad** | Kurk, tomat (kuiv, kuum) | Kollakad täpid, peenike võrk | Niisuta õhku, pese veega |

### Levinumad haigused

| Haigus | Toidutaimed | Tunnused | Ennetamine/tõrje |
|--------|-------------|----------|------------------|
| **Jahukaste** | Kurk, kõrvits, herned | Valge jahune kate lehtedel | Hea õhuring, ära kasta lehti, sooda+seebi lahus |
| **Lehemädanik** | Tomat, kartul | Pruunid laigud lehtedel, levib kiiresti | Ära istuta kõrvuti! Eemalda haigeid lehti, vask |
| **Hallhallitus** | Maasikad, tomatid, salatid | Hallitanud viljad/lehed | Hea õhuring, multš, eemalda haigeid osi |
| **Juuremädanik** | Kõik (liigniiskes mullas) | Närbuvad taimed, pruunid juured | Hea drenaaž, ära kasta üle |
| **Viirushaigused** | Kurgid, tomatid, kõrvitsad | Moonutatud, kirvjad lehed | Eemalda lehetäid, haigeid taimi |
| **Bakteriaalhaigused** | Tomatid, paprikad | Laigud lehtedel/viljadel | Ära kasta lehti, desinfitseeri tööriistad |

### Ökoloogilised tõrjevahendid

| Vahend | Mille vastu | Kuidas kasutada |
|--------|-------------|-----------------|
| **Seebivesi** | Lehetäid, lestad | 1 sl nõudeseepi / 1 L vesi, pihusta |
| **Neebiõli** | Lehetäid, jahukaste, lestad | 5 ml / 1 L vesi + tilk seepi, pihusta |
| **Küüslaugulahus** | Lehetäid, seenhaigused | 4–5 küünt leotada 1 L vees 24h, pihusta |
| **Sooda+seepi lahus** | Jahukaste | 1 tl sooda + tilk seepi / 1 L vesi |
| **Bacillus thuringiensis** | Röövikud (kapsauss, kartulimardika vastne) | Bioloogiline tõrjevahend, vastavalt juhendile |
| **Püretriinisprei** | Mitmesugused putukad | Viimane variant, tõhus aga tapab ka kasulikud |
| **Vaskpreparaat** | Seenhaigused | Ennetav, varakevadel |

### Kasulikud putukad

Meelita need aeda ja nad aitavad kahjureid kontrolli all hoida:

| Putukas | Kasu | Kuidas meelitada |
|---------|------|------------------|
| **Lepatriinud** | Söövad lehetäisid | Leia lahtine multš, lilled |
| **Kiilassilmad** | Söövad lehetäisid, lehtlaiku vastseid | Till, porgand, petersell õitsemas |
| **Jooksiklased** | Söövad nälkjaid, teolisi, vastseisi | Kivivarjualused, multš |
| **Sipelgad** | Segased – söövad ka kahjureid, aga "karjatavad" täisid | (sõltub kontekstist) |
| **Mesilased, kimalased** | Tolmeldamine! | Lillede mitmekesisus, pune, kurgirohi |
| **Ämblikud** | Söövad kärbseid, putukaid | Ära häiri, jäta varjupaiku |

### Ennetamise põhimõtted

1. **Külvikord** – sama kultuuri mitte samasse kohta 3–4 aasta jooksul
2. **Kaaslaskultuurid** – sibul porgandi kõrvale, basiilik tomatite vahele
3. **Hea õhuring** – hõrenda, ära istuta liiga tihedalt
4. **Õige kastmine** – mulda, mitte lehtedele; hommikul
5. **Terved taimed** – hästi väetatud ja kastatud taim on vastupidavam
6. **Hügieen** – eemalda haigeid taimi kohe, ära komposti
7. **Multšimine** – hoiab mulda niiske, vähendab pritsmeid lehtedele
8. **Varane avastamine** – kontrolli taimi regulaarselt (kord nädalas)

### Kahjurite seire kalender

| Kuu | Mida jälgida |
|-----|--------------|
| Aprill–mai | Lehetäid (herned, oad), maakirbud (redis) |
| Mai–juuni | Kapsauss, porgandikärbes, sibulakärbes |
| Juuni–august | Lestad (kuumaga), kartulimardikad, nälkjad |
| Juuli–september | Hallhallitus (marjad), jahukaste |
| Kogu hooaeg | Lehetäid, nälkjad |

---

## Märkmed

### 2025 hooaeg – õppetunnid

**TOMATID:**
- Taimed suured ja uhked, viljad ilusti küljes
- ⚠️ **Probleem:** Värvi muutsid kehvasti, pidid toas järelvalmima
- ✓ **Head sordid:** Keskmise mõõdu punane ja kollane (ovaalse kujuga)
- ✗ **Halvad sordid:** Viinamarjatüüpi sordid – kehv tulemus
- 💡 **Õppetund:** Varem üle vaadata – millised harud kannavad, lehed/õied maha

**MAASIKAD:**
- ⭐ VÄGA suured marjad: Asia ja veel üks sort
- Tippaeg: 13.–20. juuli
- Viimased marjad: juuli lõpp
- Augustis puhastatud – sai poole hõredamaks (liiga tihe oli!)

**KÜÜSLAUK:**
- Koristatud 30. juuli (lehed kolletamas)
- Küüned VÄGA suured! ⭐
- Ümarad üheküünelised → sügisel tagasi mulda
- Augustis kuivanud, lehevarred maha lõigatud

**HERNED:**
- Koristatud 9. august
- Pärast külvatud haljasväetiseks: inkarnaatristik, keerispea, taikon

**KARTUL (Solist, Annagrete):**
- Istutatud 18. mai (2,5 vagu, varajane)
- Koristatud 9. august, saak keskmine
- Pärast külvatud haljasväetis

**KURGID:**
- Esimene avamaakurk: 30. juuli ('Andrus', 'Alex')
- Ronikurgid hoo sisse saanud, otsast õitsevad
- **'Suyu Long'** (kasvuhoones): 4–5 kurki, esimene 30 cm, maitsev! ⭐

**KÕRVITSAD:**
- Esimene suvikõrvits ja patisson: ~9. august
- Sordid: 'Black Beauty' (suvikõrvits), 'Disco' (patisson)

**SIBULAD:**
- ⚠️ **Probleem:** Väga nirud, segusort sama suured kui istutades
- 💡 Vajab paremat sorti või hooldust

**TŠILLID:**
- Taimed näisid alguses nirud
- Augustis õitsema, esimesed kaunad kahel taimel

**PORRU:**
- Istutatud 25. mai

**FÜÜSAL:**
- Istutatud 29. mai

**MUUD KULTUURID:**
- Mädarõigas üles võetud
- Õunapuud ja pihlakas: lehed nirud ja kolletavad, aga õunad siiski küljes

---

### 2025 hooaeg – ajajoon

| Kuupäev | Tegevus |
|---------|---------|
| 18. mai | Varajane kartul maha (Solist, Annagrete) |
| 25. mai | Kurgid kasvuhoonesse, porru peenrale |
| 29. mai | Füüsal välja, porgandi ja salati külv, arbuus, till |
| 13. juuni | Kõrvitsad, kurgid, viimased tomatid ja tšillid peenrale |
| 13.–20. juuli | Maasikate tippaeg ⭐ |
| 18. juuli | Maasikad VÄGA suured (Asia jt) |
| 30. juuli | Küüslaugud üles (väga suured!), esimene avamaakurk, viimased maasikad |
| 9. august | Herned üles, kartulid üles, maasikad puhastatud, haljasväetis külvatud |
| 30. august | Tomateid oleks pidanud varem lõikama! |
| 15. sept | Tomatid täis, aga värvi ei võta – korjata ja toas valmima |

---

### 2025 hooaeg – sordid mis toimisid

| Kultuur | Head sordid | Halvad sordid |
|---------|-------------|---------------|
| Tomat | **'Malle'** ⭐ (keskm. punane), **'Pille'** ⭐ (keskm. oranž), 'Erk F1' | Viinamarjatüüpi |
| Maasikas | **Asia** ⭐ (väga suur!) | – |
| Hernes | 'Looming' ⭐⭐⭐, 'Aamisepp' | – |
| Küüslauk | Eesti sort ⭐ (suured küüned) | – |
| Kurk (avamaa) | 'Andrus', 'Alex' | – |
| Kurk (kasvuhoone) | **'Suyu Long'** ⭐ (30 cm, maitsev!) | – |
| Kõrvits | 'Black Beauty', 'Disco' | – |
| Kartul | 'Solist', 'Annagrete' | – |
| Porgand | 'Amsterdam 3', 'Naomi', 'Jerada F1' | – |
| Sibul | – | Segusort (ei kasvanud!) |

---

### 2025 hooaeg – mida teha 2026 teisiti

1. **Tomatid:**
   - Varem üle vaadata ja lõigata (juulis!)
   - Eemaldada mittekandevad harud
   - Lehed ja õied, mis enam vilja ei anna → maha
   - Valida keskmised ovaalsed sordid, mitte viinamarjatüüpi

2. **Sibulad:**
   - Proovi **'Sturon'** või **'Hercules'** (mitte 'Stuttgarter' segusort!)
   - Osta kvaliteetsest aianduskeskusest

3. **Maasikad:**
   - Hoida hõredamana (30–35 cm vahe!)
   - Asia on super – jätkata sellega

4. **Haljasväetis pärast varajasi kultuure:**
   - Herned/kartul üles → inkarnaatristik + keerispea + taikon
   - Hea praktika jätkata!

5. **Tšillid:**
   - Võib-olla varasem ettekasvatamine
   - Või soojemasse kohta

---

### 2026 hooaeg
- (Lisa siia hooaja tähelepanekud)

